...
(ÉPOCA: HISTÓRICO 2009 - VERSIÓN DE PRODUCCIÓN FOTORREALISTA V2.0)
=============================================================================
Inmueble continuo Neocolonial / Colonial Californiano en Tecate, B.C.:
  - Fachada Este (Cárdenas): 13 arcos frontales continuos y 14 pilastras de laja.
  - Galería porticada transitable en Planta Baja (X in [0.00, 2.40 m]).
  - Cancelerías retranqueadas y locales históricos de 2009:
      Santander (1-4), La Michoacana (5-6), Escalera Central (7),
      Casa Musical Tecate (8), Óptica San Martín (9), Clínicas (10-11),
      Telas y Novedades Vero (12), Regalos Brisa (13).
  - Planta Alta: Terraza corrida con barandales de herrería, ventanales en arco
    y rotulación comercial histórica 2009.
  - Cubierta inclinada con aleros volados, canes de madera vista, limaoyas y tejas cerámicas 3D.
  - Fachada Norte (Callejón Libertad): 3 arcos adosados a pared (ciegos y cajero ATM).
  - Fachada Sur (Av. Hidalgo): 2 arcos adosados a pared (Regalos Brisa y medidores).
  - La Parrilla Restaurant Bar & Grill a detalle completo: espadaña misional ondulada,
    rótulo cursivo rojo 3D, porche en arco, 4 ventanas rústicas y copete secundario.
  - Estacionamiento accesible desde Callejón Libertad con pavimento mixto y barda perimetral.
  - Zócalo basal enterrado continuo (Z = -1.50 a 0.00 m) anti-flotación topográfica.
  - Iluminación ambiental diurna y sol calibrado para renders nítidos en todas las caras.
  - Física analítica transitable en Godot 4 (.tscn) con cero barreras invisibles.
=============================================================================
"""
...
def add_arch_spandrel(bm, x_min, x_max, y_start, y_end, z_spring, z_crown, z_top, segments=14):
    """Construye un arco rebajado con dovelas y rellena las enjutas superiores hasta z_top."""
    span = y_end - y_start
...
def add_sloped_roof_hip_end(bm, x_eave_front, x_ridge, y_eave_end, y_ridge_hip, z_eave, z_ridge, is_north=True):
    """
    Construye el faldón inclinado de cierre testero (hip end) en Norte (Libertad) o Sur (Hidalgo).
    Soldado herméticamente a la limaoya y al plafón.
    """
    # Vértices de la superficie superior
    ...
    if is_north:
        ...
        bm.faces.new((v_corner_front, v_soff_front, v_soff_back, v_corner_back)) # Fascia testera
    else:
...
def make_pbr(name, base_color, roughness=0.85, metallic=0.0, alpha=1.0, tex_prefix=None, use_tex_albedo=True):
    """Crea un material Principled BSDF calibrado con enlace opcional de texturas existentes."""
    ...
    # Enlace de texturas PBR existentes si están disponibles
    if tex_prefix:
...
def create_materials():
    ...
    mats["vidrio_oscuro"] = make_pbr("M_Vidrio_Oscuro_ATM", (0.15, 0.16, 0.18), roughness=0.10, alpha=0.80)
    ...
    mats["michoacana_verde"] = make_pbr("M_Michoacana_Verde", (0.15, 0.65, 0.25), roughness=0.30)
    ...
    mats["barda_estacionamiento"] = make_pbr("M_Barda_Mamposteria", (0.70, 0.68, 0.64), roughness=0.90)
...
def build_front_cardenas(mats, col):
    """Construye las 13 crujías frontales sobre Pdte. Lázaro Cárdenas (Y in [0.00, 72.00 m])."""
    objects = []
    bay_count = 13
    total_y = 72.00
    bay_w = total_y / bay_count # ~5.538 m
    pilar_w = 0.60
    pilar_d = 0.60
    ...
    for i in range(bay_count):
        ...
        elif i == 6: # Escalera central a planta alta
            bm_esc = bmesh.new()
            step_count = 18
    ...
    bm_sant = bmesh.new()
    add_box(bm_sant, -0.22, 0.15, 6.00, 10.60, 2.50, 3.25)
    obj_s_box = create_mesh_object("Santander_Caja_Marquesina", bm_sant, mats["santander_rojo"], col)
    objects.append(obj_s_box)
    t_sant = add_3d_text("Santander_Txt_Frontal", "Santander", 0.50, 0.04, (-0.24, 8.40, 2.82), rot_cardenas, mats["santander_blanco"], col)
    objects.append(t_sant)

    # Tótem Santander en banqueta (Arco 4)
    bm_totem = bmesh.new()
    add_box(bm_totem, -1.80, -1.65, 19.80, 20.00, 0.00, 5.80)
    add_box(bm_totem, -1.85, -1.60, 19.30, 20.50, 4.30, 5.75)
    ...
    bm_mich = bmesh.new()
    add_box(bm_mich, -0.08, 0.08, 23.20, 32.80, 3.25, 3.75)
    obj_mich = create_mesh_object("Michoacana_Panel_Fascia", bm_mich, mats["michoacana_blanco"], col)
    objects.append(obj_mich)
    t_m1 = add_3d_text("Michoacana_Txt_Paleteria", "LA MICHOACANA", 0.38, 0.03, (-0.10, 28.00, 3.55), rot_cardenas, mats["michoacana_rosa"], col)
    t_m2 = add_3d_text("Michoacana_Txt_Sub", "PALETERIA  Y  NEVERIA", 0.22, 0.02, (-0.10, 28.00, 3.35), rot_cardenas, mats["michoacana_azul"], col)
    objects.extend([t_m1, t_m2])

    # Casa Musical Tecate: Rótulo sobre Arco 8
    bm_mus = bmesh.new()
    add_box(bm_mus, -0.06, 0.08, 39.20, 43.80, 2.95, 3.35)
    obj_mus = create_mesh_object("CasaMusical_Panel", bm_mus, mats["casa_musical_amarillo"], col)
    objects.append(obj_mus)
    t_mus = add_3d_text("CasaMusical_Txt", "GUITARRAS\nCASA MUSICAL TECATE", 0.22, 0.025, (-0.08, 41.50, 3.12), rot_cardenas, mats["casa_musical_letras"], col)
    objects.append(t_mus)

    # Óptica San Martín: Rótulo en bandera sobre Arco 9
    bm_opt = bmesh.new()
    add_box(bm_opt, -0.45, 0.05, 45.80, 48.40, 3.20, 3.65)
    obj_opt = create_mesh_object("Optica_Caja_Bandera", bm_opt, mats["optica_blanco"], col)
    objects.append(obj_opt)
    t_opt = add_3d_text("Optica_Txt", "OPTICA\nSAN MARTIN", 0.20, 0.02, (-0.46, 47.10, 3.40), rot_cardenas, mats["optica_azul"], col)
    objects.append(t_opt)

    # Consultorios: Rótulo sobre Arco 10
    t_dent = add_3d_text("Dentista_Txt", "DENTISTA / GINECOLOGA", 0.22, 0.02, (2.36, 52.60, 3.10), rot_cardenas, mats["clinica_azul"], col)
    objects.append(t_dent)

    # Telas y Novedades Vero: Rótulo sobre Arco 12
    bm_vero = bmesh.new()
    add_box(bm_vero, -0.06, 0.08, 61.20, 66.20, 3.25, 3.75)
    obj_vero = create_mesh_object("TelasVero_Panel", bm_vero, mats["brisa_blanco"], col)
    objects.append(obj_vero)
    t_v1 = add_3d_text("TelasVero_Txt1", "TELAS Y NOVEDADES", 0.24, 0.02, (-0.08, 63.70, 3.58), rot_cardenas, mats["telas_vero_rojo"], col)
    t_v2 = add_3d_text("TelasVero_Txt2", "VERO", 0.35, 0.03, (-0.08, 63.70, 3.38), rot_cardenas, mats["telas_vero_celeste"], col)
    objects.extend([t_v1, t_v2])

    # Regalos Brisa: Rótulo monumental sobre Arco 13
    bm_brisa = bmesh.new()
    add_box(bm_brisa, -0.10, 0.12, 66.60, 71.80, 3.25, 3.90)
    obj_brisa = create_mesh_object("RegalosBrisa_Panel", bm_brisa, mats["brisa_blanco"], col)
    objects.append(obj_brisa)
    t_b1 = add_3d_text("Brisa_Txt1", "REGALOS BRISA", 0.38, 0.03, (-0.12, 69.20, 3.65), rot_cardenas, mats["brisa_azul"], col)
    t_b2 = add_3d_text("Brisa_Txt2", "ROPA DE BAUTIZO • RECUERDOS • RAMOS • PRIMERA COMUNION", 0.13, 0.015, (-0.12, 69.20, 3.40), rot_cardenas, mats["brisa_azul"], col)
    ...
    # Salamandra (Arco 1)
    bm_sal = bmesh.new()
    add_box(bm_sal, 2.34, 2.38, 1.00, 4.60, 4.60, 5.50)
    obj_sal = create_mesh_object("Salamandra_Panel", bm_sal, mats["salamandra_amarillo"], col)
    objects.append(obj_sal)
    t_sal = add_3d_text("Salamandra_Txt", "FRACCIONAMIENTO\nLA SALAMANDRA\nLOTES EN ABONOS", 0.20, 0.02, (2.32, 2.80, 5.05), rot_cardenas, mats["salamandra_letras"], col)
    objects.append(t_sal)

    # Se Renta (Arco 2)
    t_rent = add_3d_text("SeRenta_Txt", "SE RENTA\n654-79-88", 0.28, 0.02, (2.35, 8.30, 5.10), rot_cardenas, mats["telas_vero_rojo"], col)
    objects.append(t_rent)

    # Grupo Siesa Guardias (Arco 3)
    bm_sie1 = bmesh.new()
    add_box(bm_sie1, 2.34, 2.38, 12.00, 15.60, 4.70, 5.50)
    obj_sie1 = create_mesh_object("Siesa1_Panel", bm_sie1, mats["siesa_azul"], col)
    objects.append(obj_sie1)
    t_sie1 = add_3d_text("Siesa1_Txt", "GRUPO SIESA\nSOLICITA GUARDIAS", 0.22, 0.02, (2.32, 13.80, 5.10), rot_cardenas, mats["siesa_dorado"], col)
    objects.append(t_sie1)

    # Grupo Siesa CCTV (Arco 4)
    bm_sie2 = bmesh.new()
    add_box(bm_sie2, 2.34, 2.38, 17.50, 21.20, 4.70, 5.50)
    obj_sie2 = create_mesh_object("Siesa2_Panel", bm_sie2, mats["siesa_azul"], col)
    objects.append(obj_sie2)
    t_sie2 = add_3d_text("Siesa2_Txt", "GRUPO SIESA\nVIDEOPORTEROS / CCTV", 0.22, 0.02, (2.32, 19.35, 5.10), rot_cardenas, mats["siesa_dorado"], col)
    objects.append(t_sie2)

    # Calavera Tattoo Studio (Arco 12)
    bm_cal = bmesh.new()
    add_box(bm_cal, 2.34, 2.38, 62.00, 65.50, 4.70, 5.50)
    obj_cal = create_mesh_object("Calavera_Panel", bm_cal, mats["calavera_negro"], col)
    objects.append(obj_cal)
    t_cal = add_3d_text("Calavera_Txt", "CALAVERA\ntattoo studio", 0.25, 0.02, (2.32, 63.75, 5.10), rot_cardenas, mats["calavera_dorado"], col)
    ...
    # Canes de madera salientes bajo el alero (confinados dentro del alero frontal)
    ...
    # Faldón inclinado de tejas coloniales (Alero X=-0.85, Z=6.35 m -> Cumbrera X=3.20, Z=7.35 m)
    ...
    for tin_y in [12.0, 28.0, 46.0, 64.0]:
...
def build_north_libertad_facade(mats, col):
    """Construye la fachada norte sobre Callejón Libertad con 3 arcos adosados a pared (Y = 0.00 m)."""
    objects = []
    ...
    # Muro base estucado (X in [0.00, 16.50 m])
    add_box(bm_muro, 0.00, 16.50, -0.15, 0.20, 0.00, 6.35)

    # 3 crujías de arcos adosados en Libertad (cada una de 5.50 m)
    for i in range(3):
        x1 = i * 5.50 + 0.40
        x2 = (i + 1) * 5.50 - 0.40
        add_arch_spandrel_x(bm_ladrillo, -0.18, -0.12, x1, x2, 2.20, 2.95, 3.25, segments=12)

        if i < 2: # Arcos 1 y 2: ciegos con zócalo de laja
            add_box(bm_muro, x1, x2, -0.16, -0.13, 0.00, 0.90)
        else: # Arco 3 (poniente): Cajero Automático Santander
            add_box(bm_atm, x1 + 0.20, x2 - 0.20, -0.16, -0.14, 0.00, 2.50)

    # Planta Alta Norte: 3 ventanales en arco y barandal
    for i in range(3):
        x1 = i * 5.50 + 0.80
        x2 = (i + 1) * 5.50 - 0.80
        add_arch_spandrel_x(bm_ladrillo, -0.16, -0.13, x1, x2, 5.10, 5.75, 6.00, segments=10)

    # Barandal norte de la terraza corrida
    add_box(bm_muro, 0.00, 16.50, -0.18, -0.14, 3.40, 4.35)
    ...
    objects.extend([obj_mn, obj_ln, obj_atm, obj_tn])

    # Rótulo Cajero Automático
    t_atm = add_3d_text("Libertad_Txt_Cajero", "CAJERO\nAUTOMATICO", 0.22, 0.02, (12.20, -0.20, 2.10), rot_north, mats["aluminio_blanco"], col)
    objects.append(t_atm)

    # Portón de servicio de reja gris (X in [16.50, 18.30 m])
    bm_reja = bmesh.new()
    add_box(bm_reja, 16.50, 18.30, -0.10, -0.05, 0.00, 2.40)
    obj_rej = create_mesh_object("Libertad_Reja_Servicio", bm_reja, mats["parrilla_reja_negra"], col)
    objects.append(obj_rej)

    return objects

def build_south_hidalgo_facade(mats, col):
    """Construye la fachada sur sobre Av. Miguel Hidalgo con 2 arcos adosados a pared (Y = 72.00 m)."""
    objects = []
    ...
    # Muro sur base (X in [0.00, 11.20 m])
    add_box(bm_muro_s, 0.00, 11.20, 71.85, 72.20, 0.00, 6.35)

    # 2 grandes arcos adosados (cada uno de 5.60 m) con escaparates de Regalos Brisa
    for i in range(2):
        x1 = i * 5.60 + 0.40
        x2 = (i + 1) * 5.60 - 0.40
        add_arch_spandrel_x(bm_dov_s, 72.18, 72.24, x1, x2, 2.20, 2.95, 3.25, segments=12)
        add_box(bm_vid_s, x1 + 0.10, x2 - 0.10, 72.16, 72.22, 0.40, 2.40)

        # PA Sur: 2 ventanales superiores
        add_arch_spandrel_x(bm_dov_s, 72.18, 72.24, x1, x2, 5.00, 5.75, 6.00, segments=10)
        add_box(bm_vid_s, x1 + 0.10, x2 - 0.10, 72.16, 72.22, 4.10, 5.60)

    # Banco de medidores eléctricos de CFE en el extremo poniente
    add_box(bm_med, 10.20, 11.00, 72.22, 72.35, 0.60, 2.10)

    # Hip return hermético de la cubierta de tejas sobre el frente sur
    add_sloped_roof_hip_end(bm_techo_s, -0.85, 3.20, 72.85, 68.80, 6.35, 7.35, is_north=False)
    ...
    obj_med_o = create_mesh_object("Hidalgo_Medidores_CFE", bm_med, mats["medidores_cfe"], col)
    obj_ts = create_mesh_object("Hidalgo_Techo_Tejas_Hip", bm_techo_s, mats["teja_colonial"], col)
    objects.extend([obj_ms, obj_ds, obj_vs, obj_med_o, obj_ts])

    # Rótulo Seguridad Comercial Tecate en PA
    t_seg = add_3d_text("Hidalgo_Txt_Seguridad", "SEGURIDAD COMERCIAL TECATE", 0.18, 0.02, (5.60, 72.25, 5.15), rot_south, mats["aluminio_blanco"], col)
    objects.append(t_seg)

    return objects

# ---------------------------------------------------------------------------
# 5. La Parrilla Restaurant Bar & Grill (Alcance a Detalle Completo)
# ---------------------------------------------------------------------------

def build_la_parrilla_complete(mats, col):
    """
    Construye con minuciosidad artesanal La Parrilla Restaurant Bar & Grill:
      - Frente sobre Callejón Libertad (X in [18.30, 30.50 m], Y = 0.00 m)
      - Espadaña ondulada monumental misional hasta Z = 5.15 m
      - Rótulo cursivo en relieve 3D 'La Parrilla' y 'Restaurant Bar & Grill'
      - Porche de acceso en esquina ochavada con zaguán en arco
      - Fachada poniente hacia el estacionamiento (X = 30.50 m, Y in [0.00, 22.00 m])
      - 4 ventanas rústicas en arco y copete secundario con entrada peatonal
    """
    objects = []
    rot_north = (math.radians(90.0), 0.0, 0.0)
    ...
    # 1. CUERPO PRINCIPAL DE LA PARRILLA
    # Zócalo basal enterrado continuo (-1.50 a 0.00 m)
    add_box(bm_muro, x_start - 0.20, x_end + 0.20, -0.20, y_depth + 0.20, -1.50, 0.00)

    # Muros perimetrales envolventes
    # Muro Sur y Muro Este (colindancias)
    add_box(bm_muro, x_start, x_end, y_depth - 0.25, y_depth, 0.00, 3.80)
    add_box(bm_muro, x_start, x_start + 0.25, 0.00, y_depth, 0.00, 3.80)

    # 2. FACHADA NORTE (CALLEJÓN LIBERTAD)
    # A. Marquesina rústica izquierda (X in [18.30, 23.50 m])
    # Muro con vano de ventana
    add_box(bm_muro, 18.30, 19.30, -0.15, 0.20, 0.00, 3.80)
    add_box(bm_muro, 22.70, 23.50, -0.15, 0.20, 0.00, 3.80)
    add_box(bm_muro, 19.30, 22.70, -0.15, 0.20, 0.00, 1.00) # Antepecho
    add_box(bm_muro, 19.30, 22.70, -0.15, 0.20, 2.40, 3.80) # Dintel

    # Ventana enrejada bajo marquesina
    add_box(bm_ventanas, 19.35, 22.65, 0.00, 0.08, 1.05, 2.35) # Marco madera
    add_box(bm_ventanas, 19.45, 22.55, 0.02, 0.06, 1.15, 2.25) # Vidrio
    # Reja de forja con barrotes verticales
    for b in range(12):
        xb = 19.45 + b * 0.28
        add_box(bm_rejas, xb - 0.015, xb + 0.015, -0.16, -0.12, 1.00, 2.40)
    add_box(bm_rejas, 19.35, 22.65, -0.16, -0.12, 1.00, 1.05) # Travesaño inf
    add_box(bm_rejas, 19.35, 22.65, -0.16, -0.12, 2.35, 2.40) # Travesaño sup

    # Canes y vigas de madera salientes de la marquesina
    for i in range(7):
        xv = 18.50 + i * 0.75
        add_box(bm_vigas, xv - 0.08, xv + 0.08, -0.80, 0.25, 2.95, 3.12)
    add_box(bm_vigas, 18.40, 23.20, -0.80, -0.65, 2.90, 3.05) # Vigueta frontal

    # B. Muro Central bajo Espadaña (X in [23.50, 28.50 m])
    add_box(bm_muro, 23.50, 28.50, -0.15, 0.20, 0.00, 3.80)

    # C. ESPADAÑA MISIONAL ONDULADA MONUMENTAL (X in [23.50, 30.50 m], Z in [3.80, 5.20 m])
    esp_pts = [
        (23.50, 3.80), (23.80, 4.10), (24.30, 4.35), (25.00, 4.55),
        (25.80, 4.70), (26.40, 4.95), (26.70, 5.15), (27.00, 5.20),
        (27.30, 5.15), (27.60, 4.95), (28.20, 4.70), (29.00, 4.55),
        (29.70, 4.35), (30.20, 4.10), (30.50, 3.80)
    ]
    num_pts = len(esp_pts)
    v_front = [bm_espadaña.verts.new((x, -0.12, z)) for x, z in esp_pts]
    v_back = [bm_espadaña.verts.new((x, 0.15, z)) for x, z in esp_pts]

    # Caras frontal y trasera de la espadaña
    bm_espadaña.faces.new(v_front)
    bm_espadaña.faces.new(reversed(v_back))

    # Canto superior e inferior
    for k in range(num_pts - 1):
        bm_espadaña.faces.new((v_front[k], v_front[k+1], v_back[k+1], v_back[k]))
    # Cierre basal
    bm_espadaña.faces.new((v_front[num_pts-1], v_front[0], v_back[0], v_back[num_pts-1]))

    # Moldura decorativa en relieve sobre el contorno
    for k in range(num_pts - 1):
        x0, z0 = esp_pts[k]
        x1, z1 = esp_pts[k+1]
        add_box(bm_espadaña, min(x0, x1) - 0.02, max(x0, x1) + 0.02, -0.16, -0.11, min(z0, z1) - 0.02, max(z0, z1) + 0.04)

    # D. PORCHE DE ACCESO EN ESQUINA OCHAVADA (X in [28.50, 30.50 m], Y in [0.00, 2.20 m])
    # Arco frontal exterior diáfano en Libertad
    add_box(bm_muro, 28.50, 30.30, -0.15, 0.15, 2.50, 3.80) # Dintel superior
    add_box(bm_muro, 28.30, 28.60, -0.15, 0.15, 0.00, 2.50) # Jamba izquierda
    add_box(bm_muro, 30.15, 30.45, -0.15, 0.15, 0.00, 2.50) # Jamba derecha

    # Muro lateral poniente del porche hacia el estacionamiento
    add_box(bm_muro, 30.30, 30.50, 0.00, 2.20, 0.00, 3.80)

    # Firme de baldosas de terracota en el porche interior
    add_box(bm_muro, 28.50, 30.35, 0.00, 2.20, 0.00, 0.06)

    # Techo de vigas del porche
    for ib in range(3):
        y_beam = 0.55 + ib * 0.60
        add_box(bm_vigas, 28.55, 30.30, y_beam - 0.06, y_beam + 0.06, 2.52, 2.65)

    # Puerta interior de acceso de madera rústica (Y = 2.20 m)
    add_box(bm_muro, 28.50, 30.30, 2.15, 2.35, 2.40, 3.80) # Dintel fondo
    add_box(bm_ventanas, 28.75, 30.05, 2.18, 2.28, 0.00, 2.35) # Puerta madera
    add_box(bm_rejas, 28.95, 29.85, 2.17, 2.29, 1.20, 2.00) # Herraje / mirilla

    # Farol colonial de forja en esquina exterior
    add_box(bm_rejas, 30.30, 30.45, -0.25, -0.15, 2.35, 2.75)
    add_box(bm_ventanas, 30.32, 30.43, -0.23, -0.17, 2.40, 2.65) # Linternilla

    # 3. FACHADA PONIENTE (ESTACIONAMIENTO - X = 30.50 m, Y in [0.00, 22.00 m])
    add_box(bm_muro, 30.35, 30.55, 2.20, y_depth, 0.00, 3.80) # Muro base continuo
    ...
    for i in range(4):
        ...
        # Vano retranqueado con marco de madera
        add_box(bm_ventanas, 30.36, 30.56, yw1, yw2, 1.15, 2.35)
        # Vidrio interior reflectante
        add_box(bm_ventanas, 30.42, 30.48, yw1 + 0.12, yw2 - 0.12, 1.25, 2.25)
        # Repisa de barro cocido
        add_box(bm_muro, 30.30, 30.65, yw1 - 0.08, yw2 + 0.08, 1.08, 1.16)
        # Reja saliente 'pecho de paloma' de forja
        add_box(bm_rejas, 30.54, 30.68, yw1, yw2, 1.15, 2.35)
        # Can de madera sobre la ventana
        add_box(bm_vigas, 30.20, 30.75, ymid - 0.09, ymid + 0.09, 3.40, 3.56)

    # Copete secundario con entrada lateral al estacionamiento (Y in [16.50, 19.50 m])
    add_box(bm_espadaña, 30.35, 30.62, 16.50, 19.50, 3.80, 4.45)
    add_box(bm_ventanas, 30.36, 30.58, 17.20, 18.80, 0.00, 2.30) # Puerta servicio

    # 4. CHIMENEA Y EQUIPOS DE COCINA EN AZOTEA
    # Tiro de chimenea de ladrillo refractario con sombrero piramidal
    add_box(bm_muro, 22.00, 24.00, 13.50, 15.50, 3.80, 5.50)
    add_box(bm_rejas, 21.85, 24.15, 13.35, 15.65, 5.50, 5.85) # Sombrerete
    # Campana de extracción y ductos
    add_box(bm_vigas, 24.50, 27.00, 15.00, 17.50, 3.80, 4.60)
    ...
    objects.extend([obj_parr_m, obj_parr_e, obj_parr_v, obj_parr_win, obj_parr_rej])

    # 5. RÓTULOS EN RELIEVE 3D DE LA PARRILLA (2009)
    t_parr1 = add_3d_text("LaParrilla_Txt_Nombre", "La Parrilla", 0.58, 0.04, (26.80, -0.15, 4.45), rot_north, mats["parrilla_rojo_letras"], col)
    t_parr2 = add_3d_text("LaParrilla_Txt_Sub", "Restaurant  Bar  &  Grill", 0.22, 0.02, (26.80, -0.15, 4.05), rot_north, mats["herreria_negra"], col)

    bm_adt = bmesh.new()
    add_box(bm_adt, 28.60, 29.00, -0.16, -0.14, 2.90, 3.30)
    obj_adt = create_mesh_object("LaParrilla_Placa_ADT", bm_adt, mats["adt_azul"], col)
    objects.extend([t_parr1, t_parr2, obj_adt])

    t_parr_lat = add_3d_text("LaParrilla_Txt_Lateral", "La Parrilla\nBar & Grill", 0.28, 0.03, (30.68, 18.00, 4.15), rot_west, mats["parrilla_rojo_letras"], col)
    objects.append(t_parr_lat)

    return objects

# ---------------------------------------------------------------------------
# 6. Explanada de Estacionamiento y Cierre Perimetral
# ---------------------------------------------------------------------------

def build_parking_and_grounds(mats, col):
    """Construye la explanada del estacionamiento y las bardas perimetrales."""
    objects = []

    bm_asf = bmesh.new()
    add_box(bm_asf, 11.20, 52.00, 0.00, 52.00, -0.05, 0.00)
    ...
    bm_barda = bmesh.new()
    add_box(bm_barda, 51.70, 52.10, 0.00, 52.00, 0.00, 2.40)
    add_box(bm_barda, 11.20, 52.00, 51.70, 52.10, 0.00, 2.40)
    obj_brd = create_mesh_object("Estacionamiento_Bardas_Perimetro", bm_barda, mats["barda_estacionamiento"], col)
    objects.append(obj_brd)

    return objects

# ---------------------------------------------------------------------------
# 7. Configuración de Cámaras de Validación Closed-Loop
# ---------------------------------------------------------------------------
...
def setup_cameras_and_render(col):
    ...
    # Entorno de cielo diurno suave (World Background) para iluminar caras sombreadas
    ...
    # Luz de relleno sur (para que Av. Hidalgo tenga lectura impecable)
    ...
    cam_configs = [
        ...
        ("Cam_Cardenas_Centro", (-28.0, 36.0, 4.5), (0.0, 36.0, 3.5), 32.0),
        ("Cam_Cardenas_Sur_45", (-20.0, 80.0, 5.8), (6.0, 58.0, 3.5), 30.0),
        ("Cam_Libertad_Norte", (8.0, -18.0, 4.0), (8.0, 0.0, 3.2), 28.0),
        ("Cam_Hidalgo_Sur", (5.6, 90.0, 4.0), (5.6, 72.0, 3.2), 28.0),
        ("Cam_Parrilla_Libertad", (24.5, -16.0, 4.0), (24.5, 0.0, 3.2), 28.0),
        ("Cam_Estacionamiento_Reverso", (44.0, 10.0, 6.0), (24.0, 18.0, 3.0), 26.0),
        ("Cam_Cenital_Top", (20.0, 36.0, 95.0), (20.0, 36.0, 0.0), 24.0),
    ]
...
# ---------------------------------------------------------------------------
# 8. Generación Programática de Escena Godot 4 (.tscn)
# ---------------------------------------------------------------------------

def generate_godot_scene(tscn_path, glb_path):
    """Genera la escena .tscn con física analítica transitable y cero barreras invisibles."""
    rel_glb = "res://assets/buildings/edificio_cardenas_25.glb"
    bay_count = 13
    total_y = 72.00
    bay_w = total_y / bay_count

    tscn_content = f"""[gd_scene load_steps=18 format=3 uid="uid://cardenas_25_complejo_001"]
...
[sub_resource type="BoxShape3D" id="BoxShape3D_muro_locales"]
size = Vector3(0.30, 3.40, 72.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_terraza_piso"]
size = Vector3(2.40, 0.20, 72.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_barandal_frontal"]
size = Vector3(0.06, 0.95, 72.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_muro_pa"]
size = Vector3(0.30, 3.20, 72.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_escalon"]
size = Vector3(0.20, 0.18, 3.80)

[sub_resource type="BoxShape3D" id="BoxShape3D_muro_norte"]
size = Vector3(16.50, 6.40, 0.30)

[sub_resource type="BoxShape3D" id="BoxShape3D_muro_sur"]
size = Vector3(11.20, 6.40, 0.30)

[sub_resource type="BoxShape3D" id="BoxShape3D_muro_trasero"]
size = Vector3(0.30, 6.40, 72.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_parrilla_cuerpo"]
size = Vector3(12.20, 4.20, 22.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_barda_poniente"]
size = Vector3(0.40, 2.40, 52.00)

[sub_resource type="BoxShape3D" id="BoxShape3D_barda_sur"]
size = Vector3(40.80, 2.40, 0.40)
...
"""
    ...
    tscn_content += f"""# 2. Cancelería de Locales Retranqueada
[node name="Col_Muro_Locales" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 2.45, 1.70, -36.00)
shape = SubResource("BoxShape3D_muro_locales")

# 3. Terraza y Barandales de Planta Alta
[node name="Col_Piso_Terraza" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 1.20, 3.40, -36.00)
shape = SubResource("BoxShape3D_terraza_piso")

[node name="Col_Barandal_Frontal" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0.00, 3.90, -36.00)
shape = SubResource("BoxShape3D_barandal_frontal")

[node name="Col_Muro_PA" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 2.45, 4.90, -36.00)
shape = SubResource("BoxShape3D_muro_pa")

# 4. Muros Perimetrales Norte, Sur y Trasero
[node name="Col_Muro_Norte" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 8.25, 3.20, 0.00)
shape = SubResource("BoxShape3D_muro_norte")

[node name="Col_Muro_Sur" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 5.60, 3.20, -72.00)
shape = SubResource("BoxShape3D_muro_sur")

[node name="Col_Muro_Trasero" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 11.20, 3.20, -36.00)
shape = SubResource("BoxShape3D_muro_trasero")

# 5. La Parrilla Restaurant Bar & Grill y Bardas de Estacionamiento
[node name="Col_Parrilla_Cuerpo" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 24.40, 2.10, -11.00)
shape = SubResource("BoxShape3D_parrilla_cuerpo")

[node name="Col_Barda_Poniente" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 51.90, 1.20, -26.00)
shape = SubResource("BoxShape3D_barda_poniente")

[node name="Col_Barda_Sur" type="CollisionShape3D" parent="."]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 31.60, 1.20, -51.90)
shape = SubResource("BoxShape3D_barda_sur")
"""
...
def main():
    print("=" * 70)
    print("INICIANDO RECONSTRUCCIÓN PROCEDURAL V2.0: PDTE. LÁZARO CÁRDENAS 25")
    print("=" * 70)
    ...
    print("-> Generando Fachada Frontal Cárdenas (13 Arcos y 14 Pilastras)...")
    objs_front = build_front_cardenas(mats, root_col)

    print("-> Generando Fachada Norte Callejón Libertad (3 Arcos pegados a pared y Cajero ATM)...")
    objs_north = build_north_libertad_facade(mats, root_col)

    print("-> Generando Fachada Sur Av. Miguel Hidalgo (2 Arcos pegados a pared y Regalos Brisa)...")
    objs_south = build_south_hidalgo_facade(mats, root_col)

    print("-> Generando La Parrilla Restaurant Bar & Grill a Detalle Completo...")
    objs_parrilla = build_la_parrilla_complete(mats, root_col)

    print("-> Generando Estacionamiento y Cierres Perimetrales...")
    objs_parking = build_parking_and_grounds(mats, root_col)

    total_objs = len(objs_front) + len(objs_north) + len(objs_south) + len(objs_parrilla) + len(objs_parking)
    ...
    print(f"Modelo de producción GLB exportado: {GLB_OUT}")

    # 4. Generar escena Godot (.tscn)
    generate_godot_scene(TSCN_OUT, GLB_OUT)

    # 5. Configurar cámaras y ejecutar validación closed-loop
    print("-> Configurando cámaras técnicas e iluminación Cycles...")
    cams = setup_cameras_and_render(root_col)
    print("-> Ejecutando batería de 8 renders de validación closed-loop...")
    execute_validation_renders(cams)

    print("=" * 70)
    print("PROCESO DE GENERACIÓN PROCEDURAL V2.0 COMPLETADO EXITOSAMENTE")
    print("=" * 70)
