...
Cumple estrictamente con las reglas de GEMINI.md y los requerimientos del usuario:
1. Recorte del borde Oeste: El parque termina exactamente donde termina el semicírculo
   del busto de Cárdenas (X = -38.5m), respetando la vía vehicular y estacionamientos de Cárdenas.
2. Macetas y árboles: Cada alcorque anular elevado tiene exactamente UN árbol en su centro.
   Cero árboles en andadores ni sobre la plaza de la Fuente.
3. Cero clipping con la Fuente: Plaza de la Fuente limpia y despejada (radio libre 7.5m),
   eliminada cualquier caseta de tejas o mesas cerca de la Fuente.
4. Paso peatonal libre: Cero farolas ni bustos en el eje central de ningún andador. Farolas
   y bancas colocadas a los costados (por dentro). Andador sur ensanchado alrededor de Hidalgo
   con pasillos laterales libres hacia Libertad.
5. Andadores simétricos: Los 4 andadores diagonales presentan el mismo ángulo canónico
   exacto (45°, 135°, 225°, 315°), sin deformación hacia el obelisco.
6. Delimitación de áreas verdes: La superficie base general del parque es pavimento de adoquín.
   Las áreas verdes son praderas elevadas confinadas por muretes amarillos ocre que rellenan
   exclusivamente los cuadrantes interiores entre andadores (rectángulo central de ~26m de radio).
7. Casetita blanca conmemorativa: Modelada fielmente en el jardín entre el andador NW y N,
   con su murete curvo escalonado, celosías superiores, marco conmemorativo y puerta lateral rústica
   (según evidencia fotográfica media_1790444944727.jpg).
8. Orientaciones de monumentos: Juárez mirando a la esquina exterior Noreste (+135°),
   Cárdenas mirando a la esquina exterior Suroeste (-45°), Hidalgo mirando al Kiosko (+Y).
9. Zócalo basal enterrado continuo (Z <= -1.50m) y prohibición de banquetas embebidas.
10. Sincronización canónica glTF/Godot: Z_godot = -Y_blender.
"""
...
mat_adoquin = create_pbr_material("M_Adoquin_Andador", (0.72, 0.71, 0.68, 1.0), 0.80, 0.0, "parque_adoquin", 0.6)
...
# ==============================================================================
# 3. TOPOGRAFÍA, ZÓCALO BASAL Y 8 ANDADORES EN ESTRELLA SIMÉTRICOS
# ==============================================================================
print("[PARQUE HIDALGO] Construyendo plataforma sólida y delimitación exacta del parque...")

# REGLA DEL USUARIO (PUNTO 1): El parque termina aproximadamente donde termina el semicírculo
# del busto de Cárdenas (X = -38.5m), respetando la vía vehicular de Cárdenas.
# Borde Este: Ortiz Rubio (X = +52.0m).
# Bordes Norte y Sur: Av. Juárez (Y = +36.5m) y Callejón Libertad (Y = -36.5m).
x_min, x_max = -38.5, 52.0
y_min, y_max = -36.5, 36.5

park_w = x_max - x_min # 90.5m
park_d = y_max - y_min # 73.0m
park_cx = (x_min + x_max) * 0.5 # 6.75m
park_cy = (y_min + y_max) * 0.5 # 0.0m

# Zócalo basal continuo Z <= -1.50m (Garantía GEMINI.md)
add_solid_box(bm, center=(park_cx, park_cy, -0.75), size=(park_w, park_d, 1.50), mat_index=idx_zocalo)

# Superficie basal general del parque: TODO ES PAVIMENTO DE ADOQUÍN / CONCRETO (Z = 0.02m)
add_solid_box(bm, center=(park_cx, park_cy, 0.01), size=(park_w - 0.2, park_d - 0.2, 0.04), mat_index=idx_adoquin, uv_scale=0.6)

# Glorieta central de adoquín alrededor del Kiosko (vano libre R=7.50m que respeta las escaleras del Kiosko)
add_solid_ring(bm, center=(0.0, 0.0, 0.03), r_ext=14.50, r_int=7.50, height=0.06, segments=40, mat_index=idx_adoquin)
add_solid_ring(bm, center=(0.0, 0.0, 0.16), r_ext=14.85, r_int=14.50, height=0.32, segments=40, mat_index=idx_ocre)

# Función de Andador Sólido Orientado
def add_oriented_walkway(start_xy, end_xy, width, mat_idx=idx_adoquin):
    ...
    add_solid_box(bm, center=(mid_x, mid_y, 0.04), size=(length, width, 0.06), rot_z=angle, mat_index=mat_idx, uv_scale=0.6)
...
# 8 Andadores radiales en estrella simétricos (PUNTOS 4 Y 5)
# 4 Ortogonales:
add_oriented_walkway((0.0, 14.5), (0.0, 36.5), 4.80)     # Norte (Av. Benito Juárez)
add_oriented_walkway((0.0, -14.5), (0.0, -36.5), 4.80)   # Sur (Callejón Libertad)
add_oriented_walkway((14.5, 0.0), (52.0, 0.0), 5.20)     # Este (Ortiz Rubio - Frente a Palacio)
add_oriented_walkway((-14.5, 0.0), (-38.5, 0.0), 5.00)   # Poniente (Cárdenas)

# 4 Diagonales simétricas canónicas (ÁNGULOS EXACTOS A 45°, 135°, 225°, 315°):
r_diag_start = 14.5
d_len = 16.0
for deg in [45.0, 135.0, 225.0, 315.0]:
    rad = math.radians(deg)
    p_start = (r_diag_start * math.cos(rad), r_diag_start * math.sin(rad))
    p_end = ((r_diag_start + d_len) * math.cos(rad), (r_diag_start + d_len) * math.sin(rad))
    add_oriented_walkway(p_start, p_end, 3.80)

# REGLA DEL USUARIO (PUNTO 6): "No todo el parque es área verde, solamente lo es el área que
# rellena entre andadores formando un rectángulo central terminando poco antes de los monumentos".
# Construimos las 4 islas ajardinadas elevadas con césped (+0.10m) y murete perimetral ocre (0.35m):
print("[PARQUE HIDALGO] Construyendo sectores ajardinados confinados entre andadores...")

def add_garden_sector(bm, center_xy, size_xy):
    cx, cy = center_xy
    sx, sy = size_xy
    # Plataforma de tierra y césped elevada
    add_solid_box(bm, center=(cx, cy, 0.08), size=(sx, sy, 0.14), mat_index=idx_cesped, uv_scale=0.35)
    # Murete perimetral amarillo ocre
    add_solid_box(bm, center=(cx, cy + sy*0.5, 0.20), size=(sx, 0.24, 0.38), mat_index=idx_ocre)
    add_solid_box(bm, center=(cx, cy - sy*0.5, 0.20), size=(sx, 0.24, 0.38), mat_index=idx_ocre)
    add_solid_box(bm, center=(cx + sx*0.5, cy, 0.20), size=(0.24, sy, 0.38), mat_index=idx_ocre)
    add_solid_box(bm, center=(cx - sx*0.5, cy, 0.20), size=(0.24, sy, 0.38), mat_index=idx_ocre)
    # Reja decorativa baja de forja sobre el murete
    add_solid_box(bm, center=(cx, cy + sy*0.5, 0.46), size=(sx, 0.04, 0.16), mat_index=idx_forja)
    add_solid_box(bm, center=(cx, cy - sy*0.5, 0.46), size=(sx, 0.04, 0.16), mat_index=idx_forja)
    add_solid_box(bm, center=(cx + sx*0.5, cy, 0.46), size=(0.04, sy, 0.16), mat_index=idx_forja)
    add_solid_box(bm, center=(cx - sx*0.5, cy, 0.46), size=(0.04, sy, 0.16), mat_index=idx_forja)

# 4 Cuadrantes de jardineras interiores (terminando antes de los monumentos exteriores):
add_garden_sector(bm, center_xy=(15.5, 15.5), size_xy=(18.0, 16.0))    # Cuadrante Noreste
add_garden_sector(bm, center_xy=(15.5, -15.5), size_xy=(18.0, 16.0))   # Cuadrante Sureste
add_garden_sector(bm, center_xy=(-16.5, -15.5), size_xy=(16.0, 16.0))  # Cuadrante Suroeste
add_garden_sector(bm, center_xy=(-16.5, 15.5), size_xy=(16.0, 16.0))   # Cuadrante Noroeste

# Explanada circular pavimentada para la Fuente de la Paz (vano libre R=7.50m coincidente con Godot)
...
# Conexión continua de adoquín entre la Fuente y el andador diagonal Noroeste
add_oriented_walkway((-24.0, 24.0), (fuente_cx, fuente_cy), 3.80)

# ==============================================================================
# 4. MODELADO DE LOS MONUMENTOS HISTÓRICOS (ORIENTACIÓN RIGUROSA)
# ==============================================================================
...
# REGLA (PUNTO 4): No obstruye el paso peatonal. Andador sur ensanchado a los lados.
# ------------------------------------------------------------------------------
hid_x, hid_y = 0.0, -22.0

# Medallón circular en el suelo (Piedra laja rústica de media_1790414613164.png)
add_solid_cylinder(bm, center=(hid_x, hid_y, 0.04), radius=2.60, height=0.08, segments=32, mat_index=idx_laja)
add_solid_ring(bm, center=(hid_x, hid_y, 0.06), r_ext=2.80, r_int=2.60, height=0.12, segments=32, mat_index=idx_cantera)

# Zócalo de mampostería de piedra rústica
add_solid_box(bm, center=(hid_x, hid_y, 0.22), size=(1.70, 1.70, 0.36), mat_index=idx_cantera)

# Pedestal blanco moldurado clásico
...
# Busto de Miguel Hidalgo en bronce dorado (Mirando al norte hacia el Kiosko)
...
# Pasillos peatonales laterales que rodean a Hidalgo hacia Callejón Libertad
add_solid_box(bm, center=(hid_x - 3.4, hid_y, 0.04), size=(2.4, 6.0, 0.06), mat_index=idx_adoquin)
add_solid_box(bm, center=(hid_x + 3.4, hid_y, 0.04), size=(2.4, 6.0, 0.06), mat_index=idx_adoquin)

# ------------------------------------------------------------------------------
# MONUMENTO 2: ESTATUA DE DON BENITO JUÁREZ (Sector Noreste)
# Orientado hacia la esquina Noreste (Av. Juárez y Ortiz Rubio, rot_z = 135°)
# ------------------------------------------------------------------------------
bj_x, bj_y = 30.0, 22.0
...
# Pedestal de cantera negra pulida
...
# Estatua de cuerpo entero de Don Benito Juárez mirando a la esquina Noreste
stat_z = 2.53
stat_ly = -0.20
...
# Caseta CROC tradicional verde detrás del podio
...
# Orientado hacia la esquina Suroeste (Cárdenas y Libertad, rot_z = -45°)
# Termina limpiamente en el borde Oeste (X = -38.5m)
# ------------------------------------------------------------------------------
lc_x, lc_y = -35.5, -24.5
...
# Muro blanco tipográfico frontal
...
# Pedestal y Busto de Lázaro Cárdenas integrado sobre el centro del muro
...
# Murete curvo frontal amarillo ocre y jardinera semicircular orientada al Suroeste
r_curv = 2.80
n_curv = 24
...
# Ubicado en la gran explanada Este frente a Ortiz Rubio (PUNTO 4: paso holgado a los lados)
# ------------------------------------------------------------------------------
ob_x, ob_y = 38.0, -2.0
...
# ------------------------------------------------------------------------------
# MONUMENTO 5: CASETITA BLANCA CONMEMORATIVA (Entre andador NW y N)
# REGLA DEL USUARIO (PUNTO 7) — Basada en media_1790444944727.jpg
# ------------------------------------------------------------------------------
print("[PARQUE HIDALGO] Construyendo casetita blanca conmemorativa y murete curvo (media_1790444944727.jpg)...")
cas_w_x, cas_w_y = -10.0, 19.5
cas_rot = math.radians(-35.0) # Orientada hacia el andador diagonal Noroeste
...
# Arranca bajo (0.40m) y asciende en curva cóncava suave hasta 0.90m con remate semicircular central
mur_pts = [
    rot_cw(-3.6, -1.8, 0.20),
    rot_cw(-2.4, -1.8, 0.35),
    rot_cw(-1.2, -1.8, 0.50),
...
# ==============================================================================
# 5. ALCORQUES ANULARES ELEVADOS Y ÁRBOLES ACOPLADOS 1:1 (PUNTOS 2 Y 3)
# ==============================================================================
...
def add_organic_tree(bm, pos_xy, trunk_h=3.5, trunk_r=0.45, crown_r=4.0, crown_h=5.5):
    tx, ty = pos_xy
    # Tronco cónico con ensanchamiento basal
...
def add_tree_with_planter(bm, center_xy, r_ext=1.60, r_int=1.25, height=0.38):
    ...
    # Árbol plantado exactamente en el centro (Garantía 1:1)
    add_organic_tree(bm, center_xy, trunk_h=3.4, trunk_r=0.40, crown_r=3.5, crown_h=5.0)

# Lista única de alcorques elevados con árbol acoplado (ubicados en bordes de andadores y explanadas)
alcorques_specs = [
...
monumental_trees_in_gardens = [
    # Cuadrante Noreste
    (14.0, 14.0), (20.0, 10.0), (10.0, 20.0),
    # Cuadrante Sureste
    (14.0, -14.0), (20.0, -10.0), (10.0, -20.0),
    # Cuadrante Suroeste
    (-14.0, -14.0), (-20.0, -10.0), (-10.0, -20.0),
    # Cuadrante Noroeste (lejos de la casetita blanca y de los andadores)
    (-14.0, 14.0), (-18.0, 8.0)
]
...
# ==============================================================================
# 6. MOBILIARIO URBANO (BANCAS Y FAROLAS A LOS COSTADOS)
# ==============================================================================
print("[PARQUE HIDALGO] Generando bancas y farolas en los costados de los andadores...")
...
def add_ornamental_bench(bm, pos_xy, angle_rad=0.0):
    ...
    add_solid_box(bm, center=rot(0.0, 0.05, 0.44), size=(1.80, 0.48, 0.04), rot_z=angle_rad, mat_index=idx_madera)

benches_specs = [
    # Andador Norte (x=0, bancas en x=±1.95m viendo al centro x=0)
    ((-1.95, 18.0), -math.pi * 0.5),
    ((1.95, 18.0), math.pi * 0.5),
    ((-1.95, 26.0), -math.pi * 0.5),
    ((1.95, 26.0), math.pi * 0.5),

    # Andador Sur (x=0, bancas en x=±1.95m viendo al centro x=0)
    ((-1.95, -16.0), -math.pi * 0.5),
    ((1.95, -16.0), math.pi * 0.5),
    ((-1.95, -28.0), -math.pi * 0.5),
    ((1.95, -28.0), math.pi * 0.5),

    # Andador Este (y=0, bancas en y=±2.10m viendo al centro y=0)
    ((18.0, 2.10), math.pi),
    ((18.0, -2.10), 0.0),
    ((28.0, 2.10), math.pi),
    ((28.0, -2.10), 0.0),

    # Andador Poniente (y=0, bancas en y=±2.00m viendo al centro y=0)
    ((-18.0, 2.00), math.pi),
    ((-18.0, -2.00), 0.0),
    ((-28.0, 2.00), math.pi),
    ((-28.0, -2.00), 0.0),
...
# Farolas coloniales de 3 brazos (PUNTO 4: A LOS COSTADOS, CERO OBSTRUCCIÓN EN EL CENTRO)
def add_triple_colonial_lamp(bm, pos_xy):
    ...
    for arm in range(3):
        ...
        ax = math.cos(ang) * 0.60
        ay = math.sin(ang) * 0.60
        add_solid_box(bm, center=(lx + ax*0.5, ly + ay*0.5, 3.65), size=(0.06, 0.06, 0.24), mat_index=idx_forja)
        fx, fy = lx + ax, ly + ay
        add_solid_cylinder(bm, center=(fx, fy, 3.82), radius=0.20, height=0.34, segments=6, mat_index=idx_forja)
        add_truncated_pyramid(bm, center=(fx, fy, 4.08), base_s=(0.34, 0.34), top_s=(0.06, 0.06), height=0.24, mat_index=idx_forja)
...
# Mesas de ajedrez / picnic de concreto ÚNICAMENTE en la explanada Este (PUNTO 3)
...
for cp in [(24.0, -6.5), (24.0, -9.5), (27.0, -6.5), (27.0, -9.5)]:
    add_chess_table(bm, cp)

# Botes de basura cilíndricos de forja
for tp in [(-2.6, 16.0), (2.6, 16.0), (-2.6, -17.0), (2.6, -17.0), (16.0, 2.6), (16.0, -2.6), (-16.0, 2.6), (-16.0, -2.6)]:
    add_solid_cylinder(bm, center=(tp[0], tp[1], 0.42), radius=0.22, height=0.85, segments=12, mat_index=idx_forja)

# Tambos azules de 200L característicos de 2009 (media_1790414493637.jpg)
...
for bdp in [(-10.0, 11.0), (10.0, 11.0), (-10.0, -11.0), (10.0, -11.0), (34.0, 3.0)]:
    add_blue_trash_drum(bm, bdp)

# ==============================================================================
# 7. FINALIZACIÓN DE MALLA Y EXPORTACIÓN GLTF
# ==============================================================================
...
# ==============================================================================
# 8. GENERACIÓN DE ESCENA GODOT 4 (.TSCN) CON COLISIONES ANALÍTICAS
...
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 33.0, 0.2, 0.0)
shape = SubResource("Shape_Andador_Este")

[node name="Col_Andador_W" type="CollisionShape3D" parent="StaticBody3D"]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, -26.5, 0.2, 0.0)
...
transform = Transform3D(0.819152, 0, -0.573576, 0, 1, 0, 0.573576, 0, 0.819152, {cas_w_x:.1f}, 1.3, {-cas_w_y:.1f})
...
# ==============================================================================
# 9. BATERÍA DE 8 CÁMARAS CON LOOK_AT FRONTAL Y RENDERIZADO CYCLES
# ==============================================================================
...
cameras_specs = [
    ...
    },
    {
        ...
        "loc": (rot_cw(0.0, -5.5, 1.65)),
        "target": (cas_w_x, cas_w_y, 1.5),
        "lens": 32.0,
