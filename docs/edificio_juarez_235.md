...
- **Integración Canónica en Escena Principal Godot (`godot_project/main.tscn`)**:
  ...
  - Transformación Canónica (con Inclinación 3D de Terreno):
    `transform = Transform3D(0.844388, -0.008554, 0.097136, 0.012013, 0.849398, -0.029635, -0.096769, 0.030813, 0.843911, -48.8735, 400.9325, -45.1705)`
  - *Calibración Geométrica y Desescalado Proporcional*:
    - **Inclinación Tridimensional de Altura (Plano Paralelo al Suelo)**: El terreno municipal en Tecate desciende hacia el río al Norte ($-Z$) y asciende hacia las montañas al Oriente ($+X$). La normal media analítica del suelo de la manzana bajo el predio es $\vec{n} = (-0.010064, 0.999292, 0.036251)$. Al alinear el eje vertical local `basis.y` con $\vec{n}$ y proyectar la dirección longitudinal $\vec{u}_x$ sobre dicho plano, la base completa del edificio asciende de forma milimétrica a lo largo de la fachada ($\Delta Y = +1.35\text{ m}$ hacia Ortiz Rubio) y hacia el fondo de la manzana ($\Delta Y = +1.07\text{ m}$ hacia el Callejón Libertad), manteniéndose rigurosamente coplanar y paralela a la superficie del suelo en cada metro de su huella.
    - **Orden Canónico de Índices de `Transform3D` en Godot 4**: En archivos `.tscn`, `Transform3D(n0, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11)` se asigna internamente a `basis.x = (n0, n3, n6)`, `basis.y = (n1, n4, n7)`, `basis.z = (n2, n5, n8)`, `origin = (n9, n10, n11)`.
    - **Desescalado Proporcional Isótropo ($S = 0.8500$)**: Ajusta la longitud nominal de $112.50\text{ m}$ a $95.63\text{ m}$ ($95.91\text{ m}$ con marquesinas), encajando en la longitud útil del frente de manzana ($96.82\text{ m}$ entre Calle Lázaro Cárdenas y Calle Pascual Ortiz Rubio).
    - **Paralelismo Analítico con la Arista Sur de la Manzana ($\theta = 6.566^\circ$, $\frac{\Delta Z}{\Delta X} = -0.1146$)**: Con $\text{basis.x.z} = -0.096769 < 0$, la fachada desciende en $Z$ conforme avanza en $X$ a la misma tasa que la calle, manteniendo paralelismo absoluto y cero invasión de vialidad.
    - **Cota de Rasante Enrasada al Suelo ($Y = 400.9325\text{ m}$)**: Enrasa el acceso suroeste a ras de la banqueta real en la posición de desplante $(-48.8735, -45.1705)$, permitiendo que todos los accesos comerciales queden transitables a cota cero en toda la longitud del complejo.
