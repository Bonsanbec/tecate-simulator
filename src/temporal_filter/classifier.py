...
class TemporalMRFSolver:
    ...
    def solve_temporal_consistency(self, 
        ...
        """
        Propagates P(2009) through the road network.
        ...
        for pano in aligned_panos:
            station_id = pano["station_id"]
            station_to_pano[station_id] = pano

        # Build adjacency graph of camera stations along road edges
        # Two stations are neighbors if they share the same road segment (edge) and are adjacent
        station_graph = nx.Graph()
        
        # Populate nodes in our station graph
        for s_id in station_to_pano:
            station_graph.add_node(s_id, p_init=station_to_pano[s_id]["temporal_probability"], p_curr=station_to_pano[s_id]["temporal_probability"])
            
        # Connect stations along the same edges that are close to each other
        # Sort stations by edge and distance
        edge_stations = {}
        for s_id, pano in station_to_pano.items():
            edge_id = pano["edge_id"]
            if edge_id not in edge_stations:
                edge_stations[edge_id] = []
            edge_stations[edge_id].append((pano["dist_along"], s_id))
            
        for edge_id, stations_list in edge_stations.items():
            stations_list.sort() # sort by distance along edge
            for i in range(len(stations_list) - 1):
                u_id = stations_list[i][1]
                v_id = stations_list[i+1][1]
                station_graph.add_edge(u_id, v_id)
                
        # Also connect stations at intersecting nodes
        # If two edges intersect, stations closest to the intersection nodes are linked
        # To simplify, we also allow general adjacency. If station_graph is disconnected, we propagate along what we can.
        
        # Iterative propagation (neighborhood relaxation)
        for _ in range(iterations):
            next_probs = {}
            for node in station_graph.nodes:
                neighbors = list(station_graph.neighbors(node))
                p_init = station_graph.nodes[node]["p_init"]
                
                if len(neighbors) == 0:
                    next_probs[node] = p_init
                else:
                    neighbor_sum = sum(station_graph.nodes[nbr]["p_curr"] for nbr in neighbors)
            ...
            for node, p_new in next_probs.items():
                station_graph.nodes[node]["p_curr"] = p_new

        # Update original panoramas with new MRF probability estimates
        filtered_panos = []
        for pano in aligned_panos:
            s_id = pano["station_id"]
            mrf_prob = station_graph.nodes[s_id]["p_curr"] if s_id in station_graph.nodes else pano["temporal_probability"]
            ...
            if mrf_prob >= 0.70:
                ...
                print(f"[Temporal Filter] Pruned non-2009 panorama {pano['pano_id']} at {s_id} (P(2009) = {mrf_prob:.3f})")
